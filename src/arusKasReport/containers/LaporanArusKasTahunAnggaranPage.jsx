...
const [automaticReportPeriodicElement, setAutomaticReportPeriodicElement] = useState()
const { id,  } = useParams()
useEffect(() => {
		const fetchData = async () => {
			try {
				setIsLoading(prev => ({...prev, reportChartofAccountEntryPeriodic: true}))
				const { data: automaticReportPeriodicElement } = await getAutomaticReportPeriodicElement({ id })
				setAutomaticReportPeriodicElement(automaticReportPeriodicElement.data)
			} finally {
				setIsLoading(prev => ({...prev, reportChartofAccountEntryPeriodic: false}))
			}
		}
		fetchData()	
  	}, [])
